import "dotenv/config"; // nếu không muốn .env thì có thể bỏ dòng này
...
export function createApp() {
  ...
  const ADMIN_TOKEN = process.env.ADMIN_TOKEN || ""; // rỗng => demo mode
  ...
  app.get("/api/health", (req, res) => {
    res.json({ ok: true, stats: getStats(), baseUrl: BASE_URL || null });
  });
  ...
  app.post("/api/admin/create", requireAdmin, (req, res) => {
    ...
    // BASE_URL có thể rỗng khi chạy Electron => client vẫn dùng relative URL
  ...
  app.get("/api/admin/export", requireAdmin, async (req, res) => {
    ...
    // dùng URL tương đối để chạy tốt cả Electron và deploy
  ...
  app.get("/q/:code", (req, res) => res.sendFile(path.join(__dirname, "public", "qr.html")));
...
// chạy bình thường khi node server.js
if (import.meta.url === `file://${process.argv[1]}`) {
  const { port } = await startServer(Number(process.env.PORT || 3000));
  console.log(`✅ V2 Online running at http://localhost:${port}`);
}
